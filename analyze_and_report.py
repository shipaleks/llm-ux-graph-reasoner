#!/usr/bin/env python3
"""
Wrapper script for running text analysis and automatically creating an improved report.
"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Run text analysis with automatic report improvement"
    )
    ...
    parser.add_argument(
        ...
        default="en",
        choices=["en", "ru"],
        help="Output language for reports (default: en)"
    )
    ...
    try:
        ...
        print(f"\n🔄 Creating improved report...")
        logger.info(f"Starting report improvement for directory: {output_dir}")
        
        improve_cmd = ["./improve_report.py", "--dir", output_dir, "--force"]
        ...
        if improve_result.returncode != 0:
            ...
            print(f"⚠️ Warning when creating improved report.")
        else:
            ...
            if improved_report_path and os.path.exists(improved_report_path):
                ...
                print(f"📊 Improved report created: {improved_report_path}")
                print("🌐 You can open it in your browser.")
            else:
                print(f"\n⚠️ Analysis completed, but the improved report was not created.")
